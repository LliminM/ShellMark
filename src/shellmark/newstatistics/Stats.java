package shellmark.newstatistics;
...
public class Stats
{
    private shellmark.program.Application myAppObject;
    ...
    */
     ...
     */
    public Stats (shellmark.program.Application appObject)
    {
        if(DEBUG)System.err.println("Constructing new Statistics...");
        myAppObject = appObject;

        if(DEBUG)
            shellmark.util.Log.message
        ...
        shellmark.analysis.classhierarchy.ClassHierarchy chierarchy = myAppObject.getHierarchy();
        ...
        while(itr.hasNext()) {
            ...
            shellmark.program.Class myClassObj = (shellmark.program.Class)itr.next();
            ...
            if(chierarchy==null)
                ...
                }catch(shellmark.analysis.classhierarchy.ClassHierarchyException exc) {
            ...
            shellmark.program.Class superclasses[] = myClassObj.getSuperClasses();
            if(superclasses!= null) {
                for(int k=0; k <superclasses.length; k++) {
                    ...
                    shellmark.program.Method[] supermethods = superclasses[k].getMethods();
            ...
            if(DEBUG) {
                ...
                }catch(shellmark.analysis.classhierarchy.ClassHierarchyException ex) {
            ...
            shellmark.program.Method[] methods = myClassObj.getMethods();
            shellmark.util.MethodID[] mids = null;
            try{
            ...
            if(methods != null){
                for(int m=0; m<methods.length; m++) {
                    ...
                    shellmark.util.MethodID smid[] = null;
                    String midclassname = (mids[m].getClassName()).replace('/', '.');
                    shellmark.util.MethodID tempmid =
                        new shellmark.util.MethodID( mids[m].getName(), mids[m].getSignature(), midclassname);
                    try {
            ...
            if(chierarchy!=null) {
                ...
                } catch(shellmark.analysis.classhierarchy.ClassHierarchyException exc) { // CC
                    System.out.println(" Exception --> " + exc);
                    System.exit(1);
                }
            }
            ...
            shellmark.program.Field field[] = myClassObj.getFields();
            ...
            /*  get the shellmark.program.Mmethod objects and store them in a record */
     ...
     *  For example: given 'shellmark.newstatistics.Stats', this method
     *  would return * 'shellmark/newstatistics'
     */
    ...
    public shellmark.program.Method getMethodEditor(String packageName, String className, String methodName)
    {
        StatisticsRecord stat = null;
        if((stat = findRecord(packageName, className)) == null)
            return null;
    ...
    */
    ...
    public shellmark.program.Application getApplicationObject()
    {
        return myAppObject;
    }
   ...
   */
   public static String[] getMetricNames(){
      return getMetricNames(shellmark.util.classloading.IClassFinder.APP_METRIC);
   }

   public static String[] getMethodMetricNames(){
      return getMetricNames(shellmark.util.classloading.IClassFinder.METHOD_METRIC);
   }

   public static String[] getClassMetricNames(){
      return getMetricNames(shellmark.util.classloading.IClassFinder.CLASS_METRIC);
   }

   private static String[] getMetricNames(int classFinderID){
      java.util.Collection appMetrics =
         shellmark.util.classloading.ClassFinder.getClassesWithAncestor
         (classFinderID);
      return (String[])(appMetrics.toArray(new String[appMetrics.size()]));
   }


    private static shellmark.metric.Metric[] getMetrics(int metricType){
        ...
        shellmark.metric.Metric obj[] =
            new shellmark.metric.Metric[metricInstances.size()];
        for(int i = 0; i < obj.length; i++){
            obj[i] = (shellmark.metric.Metric)(metricInstances.get(i));
        }
        return obj;
    }
    /**
     ...
     */
    public static shellmark.metric.Metric[] getApplicationMetrics(){
        return getMetrics(shellmark.util.classloading.IClassFinder.APP_METRIC);
    }

    /**
     *  returns the Class level metric objects
     */
    public static shellmark.metric.Metric[] getClassMetrics(){
        return getMetrics(shellmark.util.classloading.IClassFinder.CLASS_METRIC);
    }

    /**
     *  returns the Method level metric objects
     */
    public static shellmark.metric.Metric[] getMethodMetrics(){
        return getMetrics(shellmark.util.classloading.IClassFinder.METHOD_METRIC);
    }
}
