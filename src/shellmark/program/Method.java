 ...
 * in a supplied class.
 * <PRE>
 *    static void addNOPs(shellmark.program.Class c) {
 *       shellmark.program.Method[] mlist = c.getMethods();
 *       java.util.Iterator it = c.methods();
 *       while (it.hasNext()) {
 *          shellmark.program.Method m = (shellmark.program.Method) it.next();
 *          System.out.println(
...
public abstract class Method extends shellmark.program.Object {
	
	private static final String CFG_KEY = "shellmark.program.Method-CFG";
	   private static final String IFG_KEY = "shellmark.program.Method-IFG";
	   private static final String SS_KEY = "shellmark.program.Method-SS";
	   ...
	   */
	    ...
	    */
	   public shellmark.program.LocalMethod copy() {
	      org.apache.bcel.generic.MethodGen mg =
	         methodGen.copy(getClassName(), getConstantPool());
	      String name;
	      do {
	         name = "M" + (int)(1e9 * shellmark.util.Random.getRandom().nextDouble());
	      } while(getEnclosingClass().getMethod(name,getSignature()) != null);
	      mg.setName(name);
	      mg.setMaxLocals();
	      mg.setMaxStack();
	      return new shellmark.program.LocalMethod(
	    ...
	    * if no method exists
	    */
	   public shellmark.program.Method getSuperMethod() {
	      //traverse up the parent classes
	      shellmark.program.Class currClass =
	         ((shellmark.program.Class)this.getParent()).getSuperClass();

	      String methodKey = constructName();

	      while (currClass != null) {
	         shellmark.program.Method superMethod =
	            (shellmark.program.Method) currClass.retrieve(methodKey);
	    ...
	    */
	   public shellmark.analysis.controlflowgraph.MethodCFG getCFG() {
	      return getCFG(true);
	   }
	   ...
	   public shellmark.analysis.controlflowgraph.MethodCFG getCFG(
	      ...
	      shellmark.analysis.controlflowgraph.MethodCFG cfg =
	         (shellmark.analysis.controlflowgraph.MethodCFG)retrieve(key);

	      if (cfg == null) {
	         cfg = new shellmark.analysis.controlflowgraph.MethodCFG(
	                                                                this, withExceptions);
	         cache(key, cfg);
	      }
	    ...
	    */
	   public shellmark.analysis.interference.InterferenceGraph getIFG() {
	      shellmark.analysis.interference.InterferenceGraph ifg =
	         (shellmark.analysis.interference.InterferenceGraph)retrieve(IFG_KEY);

	      if (ifg == null) {
	         ifg = new shellmark.analysis.interference.InterferenceGraph(this);
	         cache(IFG_KEY, ifg);
	      }
	    ...
	    */
	   public shellmark.analysis.stacksimulator.StackSimulator getStack() {
		   shellmark.analysis.stacksimulator.StackSimulator ss =
	         (shellmark.analysis.stacksimulator.StackSimulator)retrieve(SS_KEY);

	      if (ss == null) {
	         ss = new shellmark.analysis.stacksimulator.StackSimulator(this);
	         cache(SS_KEY, ss);
	      }
