 ...
 *
 * <P> For more background, see the {@link shellmark.program} package overview.
 *
 * <P>Here is a simple example that reads a jar file and prints the classes:
 * <PRE>
 *    shellmark.program.Application app =
 *       new shellmark.program.Application("program.jar");
 *    java.util.Iterator it = a.classes();
 *    while (it.hasNext()) {
 *       shellmark.program.Class cls = (shellmark.program.Class) it.next();
 ...
 * <PRE>
 *    shellmark.program.Application app =
 *       new shellmark.program.Application("old.jar");
 *    shellmark.program.Class c = app.getMain();
 *    c.setName("Start");
 *    app.setMain(c);
 *    app.save("new.jar");
 * </PRE>
...
public class Application extends shellmark.program.Object {
	...
	*/
	...
	public Application(java.io.File file) throws java.lang.Exception{
		...
		try {
			...
			manifest = new java.util.jar.Manifest();
	         ...
	         new shellmark.program.LocalClass(this, instream, file.getName());
	         instream.close();
		}
		...
		for (java.util.Enumeration e = jf.entries(); e.hasMoreElements(); ) {
	         ...
	         if (! je.isDirectory()) {
	            ...
	            if (fname.endsWith(".class")) {
	               new shellmark.program.LocalClass(this, instream, fname);
	            } else if (! unwanted(fname)) {
	               new shellmark.program.File(this, fname, instream);
	            }
	            instream.close();
	         }
	      }
	 ...
	 *  (which become invalid when shellmark modifies the classes).
	 */
	...
	private void findMain() {
		shellmark.program.Class mainclass = null;
		int nmains = 0;
		for (java.util.Iterator it = classes(); it.hasNext(); ) {
			shellmark.program.Class c = (shellmark.program.Class) it.next();
			shellmark.program.Method m =
	 ...
	 public void save(java.io.OutputStream ostream) throws java.io.IOException{
		 ...
		 if (atts.size() == 0 && manifest.getEntries().size() == 0) {
			...
			// include manifest in jar file
			 if (atts.getValue(java.util.jar.Attributes.Name.MANIFEST_VERSION) == null) {
	    ...
	    // override close() to avoid trouble with the multi-file jar output.
		 ...
		 shellmark.program.Object[] members = getMembers();
	 ...
	 /**
	  * Calls {@link shellmark.program.Application#close() close}.
	  */
	 ...
	 public shellmark.program.Class getMain(){
		 java.util.jar.Attributes att = manifest.getMainAttributes();
		 if(att == null) {
			 return null;
		 }
		 String classname = (String) att.get(java.util.jar.Attributes.Name.MAIN_CLASS);
		 if(classname == null) {
			 return null;
		 }else {
	 ...
	 public void setMain(shellmark.program.Class c) {
		 if(c == null) {
			 setMain((String) null);
		 }else {
			 setMain(c.getName());
	 ...
	 public void setMain(String classname) {
	    ...
	    */
	 public shellmark.program.Class findClass(String name) {
		 {
			 shellmark.program.Class cls = getClass(name);
			 if(cls != null)
				 return cls;
		 }
	 ...
	 public shellmark.program.Class getClass(String name){
		 shellmark.program.Object o = getMember(name);
		 if(o!=null && o instanceof shellmark.program.Class) {
			 return (shellmark.program.Class) o;
		 }else {
			 return null;
		 }
	 }
	 ...
	 public shellmark.program.Class[] getClasses(){
		 return (shellmark.program.Class[])
		    shellmark.util.Misc.buildArray(classes(), new shellmark.program.Class[0]);
	 }
	   /**
	 ...
	 public shellmark.program.File getFile(String name){
		 shellmark.program.Object o = getMember(name);
		 if(o!=null && o instanceof shellmark.program.File) {
			 return(shellmark.program.File) o;
		 }else {
			 return null;
		 }
	    ...
	    *
	    * @return an array of shellmark.program.File objects
	    */
	 
	 public shellmark.program.File[] getFiles(){
		 return(shellmark.program.File[])
				 shellmark.util.Misc.buildArray(files(),new shellmark.program.File[0]);
	 }
	  ...
	  * excluding class files and the manifest.
	  *
	  * @return an iterator of shellmark.program.File objects
	  */
	 
	 public java.util.Iterator files() {
		 return shellmark.util.Misc.instanceFilter(members(), shellmark.program.File.class);
	 }
	 ...
	 public shellmark.analysis.classhierarchy.ClassHierarchy getHierarchy(){ // (String jarfile) {
	      shellmark.analysis.classhierarchy.ClassHierarchy ch =
	         (shellmark.analysis.classhierarchy.ClassHierarchy) retrieve("HIERARCHY");
	      if (ch == null) {
	         ch = new shellmark.analysis.classhierarchy.ClassHierarchy(this);
	         cache("HIERARCHY", ch);
	      }
	 ...
	 public shellmark.newstatistics.Stats getStatistics() {
		 shellmark.newstatistics.Stats stats = (shellmark.newstatistics.Stats) retrieve("STATISTICS");
		 if(stats == null) {
			 stats = new shellmark.newstatistics.Stats(this);
	 ...
	 public static void main(String args[])
	   {
	      try{
	         shellmark.program.Application app = new shellmark.program.Application(new String("test.jar"));
	         shellmark.analysis.classhierarchy.ClassHierarchy ch1 =app.getHierarchy();
	   ...
	   */
	   public boolean saveUserConstraints() throws java.io.IOException{
	      ...
	      if(configFile != null){
	         ...
	         shellmark.program.UserObjectConstraints.writeUserConstraints(out, this);
	         out.close();
	         return true;
	      }
	   ...
	   private boolean loadUserConstraints() throws java.lang.Exception{
	      ...
	      if(configFile != null){
	         ...
	         if(configFile.exists()){
	            shellmark.program.UserObjectConstraints.readUserConstraints
