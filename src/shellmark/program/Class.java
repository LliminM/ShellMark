 ...
 * object in a Shellmark program object.
...
public abstract class Class extends JarElement  {
	// INVARIANT: We always have a JavaClass or ClassGen, but never both.
	   ...
	   private shellmark.program.ConstantPoolGen cpg;
	   ...
	   abstract shellmark.program.ConstantPoolGen makeCPG(
	      shellmark.program.Class c, org.apache.bcel.classfile.ConstantPool g);
	   abstract shellmark.program.Field makeField(
	      shellmark.program.Class c, org.apache.bcel.classfile.Field f);
	   abstract shellmark.program.Method makeMethod(
	   ...
	   /*package*/ Class(shellmark.program.Application parent,
	      ...
	      if (! (this instanceof shellmark.program.LibraryClass)) {
	      ...
	      // add shellmark subclass of ConstantPoolGen to catch changes
	      cpg = makeCPG(this, jclass.getConstantPool());

	      // create shellmark objects for fields
	      for (int i = 0; i < jfields.length; i++) {
	         makeField(this, jfields[i]);
	      }

	      // create shellmark objects for methods
	   ...
	   private void updateJavaClass() {
	      ...
	      shellmark.program.Method[] m = getMethods();
	      ...
	      shellmark.program.Field[] f = getFields();
	   ...
	   private shellmark.program.Class find(String classname) {
	    ...
	    */
	   public shellmark.program.LocalClass copy() {
	       needClass();
	      updateJavaClass();
	      ...
	      String name;
	      do {
	          name = "C" + (int)(1e9 * shellmark.util.Random.getRandom().nextDouble());
	      } while(find(name) != null);
	      cg.setClassName(name);
	      return new shellmark.program.LocalClass(
	    ...
	    */
	   /*package*/ void add(shellmark.program.Object o) {
	      mark();
	      needGen();
	      if (o instanceof shellmark.program.Field) {
	         addField((shellmark.program.Field)o);
	      } else { // must be Method
	         addMethod((shellmark.program.Method)o);
	      }
	   }
	    ...
	    */
	   /*package*/ void delete(shellmark.program.Object o) {
	      mark();
	      if (o instanceof shellmark.program.Field) {
	         removeField((shellmark.program.Field) o);
	      } else { // must be Method
	         removeMethod((shellmark.program.Method) o);
	      }
	   }
	    ...
	    * {@link shellmark.program.util.Renamer shellmark.program.util.Renamer}.
	    *
	    ...
	    */
	   public shellmark.program.Method getMethod(String name, String sig) {
	      return (shellmark.program.Method) getMember(name + sig);
	   }
	    ...
	    * of this class
	    */
	   public shellmark.program.Method[] getMethods() {
	      return (shellmark.program.Method[])
	    		  shellmark.util.Misc.buildArray(
	            methods(), new shellmark.program.Method[0]);
	   }
	    ...
	    *
	    * @return an iterator of <code>shellmark.program.Method</code> objects
	    */
	   public java.util.Iterator methods() {
	      return shellmark.util.Misc.instanceFilter(
	         members(), shellmark.program.Method.class);
	   }
	    ...
	    */
	   public shellmark.program.Field getField(String name,String signature) {
	      // this will find only fields,
	      // because method names include a signature
	      return (shellmark.program.Field) getMember(name + "(" + signature + ")");
	   }
	    ...
	    */
	   public shellmark.program.Field[] getFields() {
	      return (shellmark.program.Field[])
	    		  shellmark.util.Misc.buildArray(
	            fields(), new shellmark.program.Field[0]);
	   }
	    ...
	    *
	    * @return an iterator of <code>shellmark.program.Field</code>
	    * objects
	    */
	   public java.util.Iterator fields() {
	      return shellmark.util.Misc.instanceFilter(
	         members(), shellmark.program.Field.class);
	   }
	...
	// (much of the documentation also comes from there)
	    ...
	    * not be found.
	    */
	   public shellmark.program.Class[] getInterfaces() {
	      String[] inames = getInterfaceNames();
	      shellmark.program.Class[] results =
	         new shellmark.program.Class[inames.length];
	    ...
	    */
	   public shellmark.program.Class getSuperClass() {
	    ...
	    */
	   public shellmark.program.Class[] getSuperClasses() {
	      java.util.List list = new java.util.ArrayList();
	      shellmark.program.Class curr = this;
	      for (;;) {
	    	  shellmark.program.Class supr = curr.getSuperClass();
	      ...
	      return (shellmark.program.Class[])
	         list.toArray(new shellmark.program.Class[0]);
	   }
	 ...
	 * @throws ClassNotFoundException 
	    */
	   public boolean instanceOf(shellmark.program.Class super_class) throws ClassNotFoundException {
	      this.needClass();
	...
	// BCEL wrapper functions for ClassGen methods, excluding duplicates of above
	   ...
	   public void addEmptyConstructor(int access_flags) {
	      ...
	      // now we must find the thing to make a shellmark Method object
	      ...
	      for (int i = 0; i < mlist.length; i++) {
	         ...
	         if (m.getName().equals("<init>") && m.getSignature().equals("()V")) {
	            new shellmark.program.LocalMethod(this, m);
	            return;
	         }
	      }
	    ...
	    */
	   public void addField(shellmark.program.Field f) {
	      mark();
	    ...
	    */
	   public void addMethod(shellmark.program.Method m) {
	      mark();
	      if (initialized) {                // if not already there
	    ...
	    */
	   public boolean containsField(shellmark.program.Field f) {
	    ...
	    */
	   public shellmark.program.Field containsField(String name,String signature) {
	    ...
	    * otherwise returns null.
	    */
	   public shellmark.program.Method containsMethod(String name,String signature) {
	    ...
	    */
	   public void removeField(shellmark.program.Field f) {
	      mark();
	    ...
	    */
	   public void removeMethod(shellmark.program.Method m) {
	      mark();
	      super.delete(m);
	   }
	   ...
	   public void replaceField(
			   shellmark.program.Field oldField, shellmark.program.Field newField) {
	      mark();
	      delete(oldField);
	      add(newField);
	   }
	   ...
	   public void replaceMethod(
			   shellmark.program.Method oldMethod, shellmark.program.Method newMethod) {
	      mark();
	      delete(oldMethod);
	      add(newMethod);
	   }
	    ...
	    * A new shellmark ConstantPoolGen is created and installed
	   ...
	   public org.apache.bcel.generic.ConstantPoolGen
	      ...
	      cpg = makeCPG(this, g);
	      shellmark.program.Method[] mlist = getMethods();
	      for (int i = 0; i < mlist.length; i++) {
	         mlist[i].setCPG(cpg);
	      }
	      shellmark.program.Field[] flist = getFields();
