...
public class NullNENullCallGenerator implements CallGenerator {
    ...
    private shellmark.program.Method mDestMG;
    private shellmark.program.Method mSrcMG;
    ...
    private shellmark.analysis.stacksimulator.Context mSrcEdgeCx;
    ...
    public void addPhantomCall(MethodCFG srcMethod,
			       Edge srcEdge,MethodCFG destMethod) {
	mSrcCFG = srcMethod;
	mSrcEdge = srcEdge;
	...
	shellmark.analysis.stacksimulator.StackSimulator ss =
	    new shellmark.analysis.stacksimulator.StackSimulator
	    (mSrcCFG);
	if(DEBUG)
	...
	mFactory =
	    ...
	    (srcMethod.method().getConstantPool());
	...
	org.apache.bcel.generic.InstructionHandle pushNullIH =
	    ...
	    (org.apache.bcel.generic.InstructionFactory.createNull
	        (org.apache.bcel.generic.Type.getType
				 ("Ljava/lang/Object;")));
	mTestBlock = new BasicBlock(mSrcCFG);
	mTestBlock.addInst(pushNullIH);
	mTestBlock.addInst(mSrcMG.getInstructionList().append
	...
	for(int i = 0 ; i < stackSize ; i++) {
	    shellmark.analysis.stacksimulator.StackData stackItems[] =
	...
	mSrcCFG.addEdge(new FallthroughEdge(mTestBlock,mCallBlock));
	mSrcCFG.addEdge(mTestBlock,mSrcEdge.dest());
	mSrcCFG.addEdge(new FallthroughEdge(mCallBlock,mRvPopBlock));
	if(mExceptionHandlerBlock != null)
	    mSrcCFG.addEdge
		(new shellmark.analysis.controlflowgraph.ExceptionEdge
		 (mCallBlock,mExceptionHandlerBlock,
		  (org.apache.bcel.generic.CodeExceptionGen [])
		  newExceptions.toArray(new org.apache.bcel.generic.CodeExceptionGen[0])));
	...
	if(mSrcEdgeIsFallthrough) {
	    ...
	    src.setFallthrough(mTestBlock);
	    mSrcCFG.addEdge(new FallthroughEdge(src,mTestBlock));
	    mRestoreAndGotoBlock.setFallthrough(dest);
	    mSrcCFG.addEdge(new FallthroughEdge(mRestoreAndGotoBlock,dest));
	} else {
	    mSrcCFG.addEdge(mSrcEdge.src(),mTestBlock);
	    mSrcCFG.addEdge(mRestoreAndGotoBlock,mSrcEdge.dest());
	}
	mTestBlock.setFallthrough(mCallBlock);
	...
	for(int i = 0 ; i < exceptions.length ; i++) {
		...
		exceptions[i].setHandlerPC
		    ((org.apache.bcel.generic.InstructionHandle)
		     mTestBlock.getInstList().get(0));
		...
		while(throwerIt.hasNext()) {
			...
			mSrcCFG.addEdge(pred,mTestBlock);
		    }
		}
	    }
	}
