...
protected shellmark.program.Method generate(
   shellmark.util.newgraph.Graph g,
   shellmark.program.Class clazz,
 ...
 */
 protected shellmark.program.Method generate(
    ...
    shellmark.util.newgraph.Graph tree = null;
    if (hamiltonian) {
       tree = shellmark.util.newgraph.Graphs.createGraph(graph.nodes(), null);
    ...
    inslist.setPositions(true);
    shellmark.program.Method method = makeFunc(inslist, clazz);
    method.removeNOPs();
    return method;
}
 ...
 */
private java.util.BitSet[] getExits(shellmark.util.newgraph.Graph graph,
				    java.util.Map nodeNumbers,
				    shellmark.util.newgraph.Graph dftree) {
...
private java.util.Vector setIncoming(
   ...
   while (i.hasNext()) {
      ...
      while (j.hasNext()) {
	    ...
	    (shellmark.util.newgraph.Edge) j.next();
...
private void genNode(
   ...
   for (java.util.Iterator j = graph.outEdges(thisNode); j.hasNext(); ) {
      outEdges++;
      shellmark.util.newgraph.Edge e = (shellmark.util.newgraph.Edge) j.next();
...
private void genIf(
   ...
   if (e.sinkNode() == nextNode) {
      e = (shellmark.util.newgraph.Edge) j.next();
   }
