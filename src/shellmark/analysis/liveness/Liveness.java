package shellmark.analysis.liveness;
...
public class Liveness {
    ...
    private shellmark.analysis.controlflowgraph.MethodCFG mCFG;
    public Liveness(shellmark.program.Method method) {
    ...
    public boolean liveAt(shellmark.analysis.defuse.DUWeb web,
                          org.apache.bcel.generic.InstructionHandle ih) {
    	shellmark.analysis.controlflowgraph.BasicBlock bb = mCFG.getBlock(ih);
    ...
    private BBInfo calcPartialBlockInfo
        ...
        for(int i = al.size() - 1 ; i >= 0 ; i--) {
            ...
            if(shellmark.analysis.defuse.ReachingDefs.isDef(ih)) {
                int lvindex =
            ...
            if(shellmark.analysis.defuse.ReachingDefs.isUse(ih)) {
                int lvindex =
                    ((org.apache.bcel.generic.IndexedInstruction)
                     ih.getInstruction()).getIndex();
    ...
    public static void main(String argv[]) throws Exception {
        ...
        for(java.util.Iterator classes = app.classes() ; classes.hasNext() ; ) {
        	shellmark.program.Class clazz =
                (shellmark.program.Class)classes.next();
            for(java.util.Iterator methods = clazz.methods() ;
                ...
                if(method.getInstructionList() == null)
                    continue;
                shellmark.analysis.defuse.ReachingDefs rd =
                ...
                for(java.util.Iterator defs = rd.defs().iterator() ;
                    ...
                    if(!(def instanceof shellmark.analysis.defuse.InstructionDefWrapper))
                        continue;
                    org.apache.bcel.generic.InstructionHandle ih =
                        ((shellmark.analysis.defuse.InstructionDefWrapper)def).getIH();
