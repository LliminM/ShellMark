package shellmark.analysis.defuse;
...
public class ReachingDefs {
   ...
   private shellmark.analysis.controlflowgraph.MethodCFG mCFG;
    ...
    public ReachingDefs(shellmark.program.Method method) {
	this(method,false);
    }
    public ReachingDefs(shellmark.program.Method method,
	...
	mCFG.consolidate();
        ...
        if(method.getInstructionList() == null)
           throw new shellmark.analysis.controlflowgraph.EmptyMethodException();
   ...
   public java.util.HashSet bitsToDefs(java.util.BitSet bits,int lvnum) {
	...
	java.util.ArrayList webs = new java.util.ArrayList();
        ...
        for(java.util.Iterator blocks = mCFG.nodes() ; blocks.hasNext() ; ) {
           shellmark.analysis.controlflowgraph.BasicBlock bb =
              (shellmark.analysis.controlflowgraph.BasicBlock)blocks.next();
   ...
   private java.util.Hashtable initializeBBInfo
	...
	while(basicBlocks.hasNext()) {
	    shellmark.analysis.controlflowgraph.BasicBlock bb = 
		(shellmark.analysis.controlflowgraph.BasicBlock)basicBlocks.next();
	...
	for(java.util.Iterator it = subGr.defs().iterator() ; it.hasNext() ; ) {
        ...
        for(java.util.Iterator it = subGr.edges() ; it.hasNext() ; ) {
            shellmark.util.newgraph.Edge e = 
                (shellmark.util.newgraph.Edge)it.next();
	...
	shellmark.program.Application app = new shellmark.program.Application(argv[0]);
	for(java.util.Iterator classes = app.classes() ; classes.hasNext() ; ) {
	    for(java.util.Iterator methods = 
		    ((shellmark.program.Class)classes.next()).methods() ; 
		methods.hasNext() ; ) {
