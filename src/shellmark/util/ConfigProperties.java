...
public class ConfigProperties {
    ...
    private static Class sTypeClasses[] = {
            ...
            new shellmark.program.Method[0].getClass(),
            new shellmark.program.Class[0].getClass(),
    };
    ...
    // CHANGE IS HERE!
    private static java.util.Hashtable<String, Integer> sTypeDescToVal;
    static {
    	sTypeDescToVal = new java.util.Hashtable<>();
    	for (int i = 0; i < sTypeDescs.length; i++) {
            sTypeDescToVal.put(sTypeDescs[i], Integer.valueOf(i));
        }
    ...
    private static java.util.Hashtable<String, Long> sPhaseDescToVal;

    static {
        sPhaseDescToVal = new java.util.Hashtable<>();
        for (int i = 0; i < sPhaseDescs.length; i++) {
            sPhaseDescToVal.put(sPhaseDescs[i], Long.valueOf(1L << i));
        }
    }
    ...
    class PropSpec {
        ...
        public Object value;
        public long phases;
        private java.util.ArrayList<Object> listeners; 
        boolean exclusive;
        java.util.List<Object> choices; 
        ...
        PropSpec(String name, String description, String dflt, int type, String phases)
            ...
            for (String phaseDesc : phaseDescs) {
                this.phases |= ((Long) sPhaseDescToVal.get(phaseDesc)).longValue();
            }
                ...
                if (value == null || (type != TYPE_STRING && value.equals(""))) {
                    ...
                    switch (type) {
                        ...
                        case TYPE_JAR:
                            ...
                            break;
                        case TYPE_PERCENT:
                            newValue = Double.valueOf(value); // 使用 Double.valueOf
                            break;
                        case TYPE_INTEGER:
                        case TYPE_RANDOM_INT:
                            newValue = Integer.valueOf(value); // 使用 Integer.valueOf
                            break;
                        case TYPE_BOOLEAN:
                            newValue = Boolean.valueOf(value); // 使用 Boolean.valueOf
                            break;
                        case TYPE_DOUBLE:
                            newValue = Double.valueOf(value); // 使用 Double.valueOf
                            break;
                        ...
                        case TYPE_CLASS:
                            ...
                            break;
                        default:
                            throw new ConfigPropertyException("Unknown type: " + type);
                    }
                }
                setValue(newValue);
            } catch (Exception e) {
                e.printStackTrace();
                throw new ConfigPropertyException("Error while setting " + name + " to " + value + ": " + e.getMessage());
            }
        }
        ...
        public void setValue(Object value) {
            ...
            if (!correctClass.isAssignableFrom(valueClass)) {
                throw new ConfigPropertyException("Invalid type for property " + name);
            }
            if ((exclusive && choices == null) || 
                (exclusive && choices != null && !choices.contains(value))) {
                throw new ConfigPropertyException(value + " is not a valid choice for property " + name);
            }
        ...
        private class Listener {
            shellmark.util.ConfigProperties configProps;
